...
# YuanquSimulate
# ============================================================
class YuanquSimulate(BaseDataset):
    def __init__(self, dataset_dir, train_or_test='train', color='rgb', cropper='random', crop_shape=None,
        ...
        super(YuanquSimulate, self).__init__()
        assert train_or_test in ['train', 'test']
        self.color = color
        self.cropper = cropper
        self.crop_shape = crop_shape
        ...
        if p.exists():
            ...
            raise Exception('Error: Please provide a txt file for ' + train_or_test + 'ing!')
        ...
        with open(p, 'r') as f:
            for i in f.readlines():
                if i.strip():
                    img1, img2, flow = i.split(',')
                    flow = flow.strip()
                    self.samples.append((Path(self.dataset_dir) / img1, Path(self.dataset_dir) / img2, Path(self.dataset_dir) / flow))
...
# YuanquLiteFlowNet
# ============================================================
class YuanquLiteFlowNet(BaseDataset):
    def __init__(self, dataset_dir, train_or_test='train', color='rgb', cropper='random', crop_shape=None,
                 resize_shape=None, resize_scale=None):
        super(YuanquLiteFlowNet, self).__init__()
        assert train_or_test in ['train', 'test']
        self.color = color
        self.cropper = cropper
        self.crop_shape = crop_shape
